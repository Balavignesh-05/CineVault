...
import { Globe2, ArrowLeft, AlertCircle, Film } from 'lucide-react';
...
export default async function CountryPage({ params }: Props) {
  ...
  try {
    ...
    errorMsg = err?.message || 'Failed to fetch movies for this country.';
  }
      ...
      <div className="container mx-auto px-4 max-w-7xl space-y-6">
        ...
        <div className="p-6 rounded-2xl bg-surface border border-border-subtle flex items-center justify-between gap-4">
          <div className="space-y-1">
            <h1 className="text-2xl md:text-3xl font-black text-white tracking-tight flex items-center gap-2">
              <Globe2 className="w-6 h-6 text-primary" /> {countryName} Cinema
            </h1>
            <p className="text-xs md:text-sm text-text-muted">
              Discover popular movies produced in {countryName}
            </p>
          </div>
        ...
        {errorMsg ? (
          <div className="p-8 rounded-2xl bg-surface border border-border-subtle text-center space-y-3 my-8">
            ...
            <h2 className="text-lg font-bold text-white">Unable to load country catalog</h2>
          ...
          <div className="p-12 rounded-2xl bg-surface border border-border-subtle text-center space-y-3 my-8">
            ...
            <p className="text-xs text-text-muted">No movies found originating from {countryName}.</p>
          </div>
        ) : (
